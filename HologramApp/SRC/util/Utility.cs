...

{
    public class Utility : Singleton<Utility>, IMath, IDebug, IMesh
    {
        ...
        */
        ...
        GameObject IMesh.GenerateGameObjectFromPrefabBetweenTwoPoints(Vector3 p, Vector3 q, string name, string prefabPath, string layer, GameObject target, Transform parent, MeshType mt)
        {
            if (mt == MeshType.Cylinder)
                return GenerateCylinderFromPrefabBetweenTwoPoints(p, q, name, prefabPath, layer, target, parent);
        ...
        GameObject GenerateCylinderFromPrefabBetweenTwoPoints(Vector3 p, Vector3 q, string name, string prefabPath, string layer, GameObject target, Transform parent, float radius=0.005f)
        {
            GameObject cyl = Resources.Load(prefabPath) as GameObject;

            Vector3 offset = p - q;
            Vector3 scale = new Vector3(radius, offset.magnitude / 2.0f, radius);
            Vector3 position = q + (offset / 2.0f);

            GameObject cylinder = Instantiate(cyl, position, Quaternion.identity);

            cylinder.transform.up = offset;
            cylinder.transform.localScale = scale;
            cylinder.layer = LayerMask.NameToLayer(layer);
            cylinder.transform.parent = parent;
            cylinder.name = name;

            Color c = cylinder.GetComponent<Renderer>().material.color;
            c.a = 0.0f;
            cylinder.GetComponent<Renderer>().material.color = c;
