...
public:
	...
	void approximate_polygon(std::vector<cv::Point2d> & _quadrilateral, std::vector<cv::Point> const & _points, uint32_t const _accuracy = 1);
	...
	uint32_t apply_data_to_out_datastructures(float * _out_quaternion, float * _out_tvec, Quaternion<double> const & _rotation, std::vector<double> const & _translation_vector, std::vector<cv::Point2d> const & _quadrilateral);
